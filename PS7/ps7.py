...
class OrTrigger(Trigger):
    def __init__(self, Trigger1, Trigger2):
        ...
        self.Trigger2=Trigger2
    def evaluate(self, story):
        if self.Trigger1.evaluate(story):
            return True
        elif self.Trigger2.evaluate(story):
            return True
        else:
            return False
...
def filterStories(stories, triggerlist):
    ...
    for i in range(len(stories)):
        for j in range(len(triggerlist)):
            if triggerlist[j].evaluate(stories[i]) and stories[i] not in result:
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType="TITLE":
        ...
        result= TitleTrigger(i)
    if triggerType="SUBJECT":
        for i in range(len(params)):
        result= SubjectTrigger(params)
    if triggerType="SUMMARY":
        for i in range(len(params)):
        result= SummaryTrigger(params)
    if triggerType="NOT":
        for i in range(len(params)):
        result= NotTrigger(params)
    if triggerType="AND":
        for i in range(len(params)):
        result= AndTrigger(params)
    if triggerType="OR":
        for i in range(len(params)):
        result= OrTrigger(params)

    triggerMap[name] = "result"; # Add new entry
    return "result"
